...
---------------------
  ...
  write grouped_rollouts.jsonl
           │
           ▼
  train_policy_qlora_grpo   (subprocess: TRL GRPO / veRL / ART)
           │
           ▼
...
class GRPOAlgorithm(_BASE):
    ...
    trainer:
        Training backend: ``trl_grpo``, ``verl``, ``agent_lightning_official``, or ``official_art_ruler``.
    transitions_dir:
    ...
    def run(
        ...
        ----------
        checkpoint_dir:
            Override the default checkpoint_dir for this round only.
        ...
        pid          = self._launch_training(grouped_file, checkpoint_dir=checkpoint_dir)
        ...
        return {
            ...
            'checkpoint_dir': str((checkpoint_dir or self.checkpoint_dir).resolve()),
    ...
    def _launch_training(self, grouped_file: Path, checkpoint_dir: Path | None = None) -> int:
        """Launch the GRPO/QLoRA training subprocess and start the watcher thread."""
        effective_ckpt = checkpoint_dir or self.checkpoint_dir
        cmd = [
            sys.executable, '-m', 'src.training.train_policy_qlora_grpo',
            '--dataset',          str(grouped_file),
            '--output-dir',       str(effective_ckpt),
    ...
    def _watcher(self, proc: subprocess.Popen, checkpoint_dir: Path | None = None) -> None:
        """
        Background daemon thread: blocks until training finishes, then:
        1. Calls ``store.add_resources()`` with the new checkpoint path.
        2. Hot-reloads the vLLM adapter (unless reload_after is False).
        ...
        if proc.returncode == 0:
            ...
            checkpoint_path = str(effective_ckpt.resolve())
            ...
            if self.reload_after:
                ...
                reload_status = 'skipped_by_request'
