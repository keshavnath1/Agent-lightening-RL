...
Standalone (e.g. test harness, notebook)::
    ...
    from lightning_bridge.grpo_algorithm    import GRPOAlgorithm
    from trainer_app.train_sft      import LightningTrainer
...
class LightningTrainer(_TRAINER_BASE):
    ...
    * Expose ``run(max_rounds)`` for a self-contained blocking loop that
    ...
    def maybe_trigger_training(
        ...
        try:
            result = self.algorithm.run(train_dataset=succeeded)
        finally:
