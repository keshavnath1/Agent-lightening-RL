...
def get_ruler_score_group() -> Callable[..., Any]:
    ...
    raise RuntimeError('Official RULER function not found. Install/update openpipe-art[backend,langgraph]>=0.4.9. ' + ' | '.join(errors))
...
def main() -> None:
    parser = argparse.ArgumentParser(description='Run official ART + RULER training path when openpipe-art is installed.')
    ...
    metadata = run_from_dataset(args.dataset, args.output_dir, args.model_name, args.judge_model, args.rollouts_per_group, args.groups_per_step, args.max_steps)
